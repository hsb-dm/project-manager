...
function taskMenu(a){ var id=S.drawerTask; ctxMenu(a,(canI.createTask()?'<button onclick="closePops();duplicateTask(\''+id+'\')">'+I.plus+'Duplicate task</button>':'')+'<button onclick="closePops();exportTaskCSV(\''+id+'\')">'+I.download+'Export task (.csv)</button>'+(canI.editTask(task(id))?'<button onclick="closePops();toggleHidden(\''+id+'\')">'+I.eye+(task(id).hidden?'Unhide task':'Hide task')+'</button>':'')+(canI.deleteTask()?'<button onclick="closePops();deleteTask(\''+id+'\')" style="color:var(--color-danger)">'+I.trash+'Delete task</button>':'')); }
